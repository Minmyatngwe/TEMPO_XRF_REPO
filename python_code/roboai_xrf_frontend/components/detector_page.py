...
def render_detector_page():
    ...
    with tabs[1]:
        ...
        with st.form("housing_form"):
            enabled = st.checkbox("Enable detector housing", value=housing["enabled"])

            c1, c2 = st.columns(2)
            name = c1.text_input("Housing name", value=housing["name"])
            aperture_shape = c2.selectbox(
                "Aperture shape",
                ["Circular", "Rectangular"],
                index=["Circular", "Rectangular"].index(housing["aperture_shape"]),
            )
            ...
            housing_material = c1.text_input(
                "Housing material", value=housing["housing_material"]
            )
            cavity_material = c2.text_input(
                "Cavity wall material", value=housing["cavity_material"]
            )
            medium = c3.text_input(
                "Inner cavity medium", value=housing["inner_cavity_medium_material"]
            )

            c1, c2 = st.columns(2)
            window_material = c1.text_input(
                "Housing window material", value=housing["window_material"]
            )
            ...
            if st.form_submit_button("Save housing", type="primary"):
                housing.update(
                    {
                        "enabled": enabled,
                        "name": name.strip(),
    ...
    with tabs[2]:
        ...
        rows = records_editor(
            ...
            key="internal_masks_editor",
            column_config=_aperture_columns(),
        )
        if st.button("Save internal masks", type="primary"):
            detector["internal_masks"] = clean_records(rows, "name")
            invalidate_simulation()
    ...
    with tabs[3]:
        ...
        rows = records_editor(
            "Detector filters",
            detector["filters"],
            key="detector_filters_editor",
            column_config=_solid_filter_columns(),
        )
        if st.button("Save detector filters", type="primary"):
            detector["filters"] = clean_records(rows, "name")
            invalidate_simulation()
    ...
    with tabs[4]:
        rows = records_editor(
            ...
            detector["collimators"],
            key="detector_collimators_editor",
            column_config=_aperture_columns(include_enabled=True),
        )
        if st.button("Save detector collimators", type="primary"):
            detector["collimators"] = clean_records(rows, "name")
            invalidate_simulation()
